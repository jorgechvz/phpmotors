...
switch ($action) {
    ...
    case 'register':
        ...
        $emailAddress = trim(filter_input(INPUT_POST, 'clientEmail', FILTER_SANITIZE_EMAIL));
        $clientPassword = trim(filter_input(INPUT_POST, 'clientPassword', FILTER_SANITIZE_FULL_SPECIAL_CHARS));
        $clientEmail = checkEmail($emailAddress);
    ...
    case 'updateInfo':
        ...
        $updateClientInfoResult = updateAccountInfo($clientFirstname, $clientLastname, $clientEmail, $clientId);

        if ($updateClientInfoResult) {
            $updateClientData = getClientById($clientId);
            // A valid user exists, log them in
            $_SESSION['loggedin'] = TRUE;
            // Remove the password from the array
            // the array_pop function removes the last
            // element from an array
            array_pop($updateClientData);
            // Store the array into the session
    ...
    case 'updatePassword':
        ...
        $hashedNewPassword = password_hash($clientPassword, PASSWORD_DEFAULT);

        $updateClientPassword = updateAccountPassword($hashedNewPassword, $clientId);
