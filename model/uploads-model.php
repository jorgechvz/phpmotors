...
function getThumbnailByInvId($invId) {
    $db = phpmotorsConnect();
    $sql = "SELECT images.imgPath, inventory.invMake, inventory.invModel FROM images JOIN inventory ON images.invId = inventory.invId WHERE images.imgPath LIKE '%-tn%' AND inventory.invId = :invId";
    $stmt = $db->prepare($sql);
